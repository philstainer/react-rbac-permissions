...
describe('getPermissionsForRole', () => {
  ...
  it('returns empty set for non-existent role', () => {
    const { getPermissionsForRole } = createPermissions(createTestConfig())
...
describe('resolvePermissions', () => {
  ...
  it('returns empty set when role does not exist in config', () => {
    const { resolvePermissions } = createPermissions(createTestConfig())
    const perms = resolvePermissions('nonexistent' as any)

    expect(perms.size).toBe(0)
    expect(perms).toBeInstanceOf(Set)
  })

  it('handles role with undefined config gracefully', () => {
    const config = {
      ...
      permissions: {
        ...
        // editor is missing from permissions
      },
    } as any
    const { resolvePermissions } = createPermissions(config)
    const perms = resolvePermissions('editor')
  ...
  it('handles inherited role that does not exist', () => {
    const config = {
      roles: ['admin', 'editor'] as const,
      resources: {
        posts: ['read'] as const,
      ...
      permissions: {
        admin: {
          can: ['posts:read'] as const,
          inherits: ['nonexistent' as any], // inherits from non-existent role
        },
        editor: {
          can: ['posts:read'] as const,
        },
    ...
    // Should still return admin's own permissions, ignoring invalid inheritance
    expect(perms.has('posts:read')).toBe(true)
    expect(perms.size).toBe(1)
  })
})
...
describe('resolveMultipleRoles', () => {
  ...
  it('handles array with non-existent roles', () => {
    const { resolveMultipleRoles } = createPermissions(createTestConfig())
    const perms = resolveMultipleRoles(['admin', 'nonexistent' as any, 'viewer'])

    // Should return permissions from valid roles only
    expect(perms.has('posts:create')).toBe(true) // from admin
    expect(perms.has('posts:read')).toBe(true) // from admin and viewer
    expect(perms.has('users:read')).toBe(true) // from viewer
  })

  it('handles array with only non-existent roles', () => {
    const { resolveMultipleRoles } = createPermissions(createTestConfig())
    const perms = resolveMultipleRoles(['invalid1' as any, 'invalid2' as any])
  ...
  it('handles mix of valid and invalid roles', () => {
    const { resolveMultipleRoles } = createPermissions(createTestConfig())
    const perms = resolveMultipleRoles(['guest', 'nonexistent' as any])

    // Should only have guest permissions
    expect(perms.size).toBe(1)
    expect(perms.has('posts:read')).toBe(true)
  })
})
...
describe('PermissionProvider', () => {
  ...
  it('provides resolved permissions to context', async () => {
    ...
    const TestComponent = () => {
      ...
      return <div data-testid="has-posts-read">{String(permissions.has('posts:read'))}</div>
    }
  ...
  it('handles user with non-existent role', async () => {
    const { PermissionProvider, usePermissions } =
      createPermissions(createTestConfig())
    ...
    const TestComponent = () => {
      ...
      return (
        <div>
          <div data-testid="permissions-size">{permissions.size}</div>
    ...
    const screen = await render(
      <PermissionProvider roles={['nonexistent' as any]}>
        <TestComponent />
      </PermissionProvider>
    )

    await expect.element(screen.getByTestId('permissions-size')).toHaveTextContent('0')
    await expect.element(screen.getByTestId('can-read')).toHaveTextContent('false')
  })

  it('handles user with only non-existent roles', async () => {
    const { PermissionProvider, usePermissions } =
      createPermissions(createTestConfig())
    ...
    const screen = await render(
      <PermissionProvider roles={['invalid1' as any, 'invalid2' as any]}>
        <TestComponent />
      </PermissionProvider>
    )

    await expect.element(screen.getByTestId('permissions-size')).toHaveTextContent('0')
    await expect.element(screen.getByTestId('can-delete')).toHaveTextContent('false')
  })

  it('handles mix of valid and non-existent roles', async () => {
    const { PermissionProvider, usePermissions } =
      createPermissions(createTestConfig())

    const TestComponent = () => {
      const { permissions, can, roles } = usePermissions()
      return (
        <div>
    ...
    const screen = await render(
      <PermissionProvider roles={['guest', 'nonexistent' as any, 'viewer']}>
        <TestComponent />
      </PermissionProvider>
    )

    await expect.element(screen.getByTestId('roles')).toHaveTextContent('guest,nonexistent,viewer')
    await expect.element(screen.getByTestId('permissions-size')).toHaveTextContent('3')
    await expect.element(screen.getByTestId('can-read')).toHaveTextContent('true')
  })
  ...
  it('handles empty roles array', async () => {
    ...
    await expect.element(screen.getByTestId('roles-count')).toHaveTextContent('0')
    await expect.element(screen.getByTestId('permissions-size')).toHaveTextContent('0')
    await expect.element(screen.getByTestId('can-read')).toHaveTextContent('false')
  })})
...
describe('usePermissions', () => {
  ...
  describe('hasAnyRole', () => {
    it('returns true when user has at least one role', async () => {
      ...
      const TestComponent = () => {
        ...
        return (
          <div data-testid="result">{String(hasAnyRole(['admin', 'guest']))}</div>
        )
      }
  ...
  describe('getPermissionsForRole (hook)', () => {
    it('returns permissions for a given role', async () => {
      ...
      const TestComponent = () => {
        ...
        const perms = getPermissionsForRole('guest')
        return <div data-testid="result">{String(perms.has('posts:read'))}</div>
      }
    ...
    it('returns all permissions for admin role', async () => {
      ...
      await expect.element(screen.getByTestId('result')).toHaveTextContent('11')
    })
  })
  ...
  describe('getRolesWithPermission (hook)', () => {
    ...
    it('returns only admin for restricted permission', async () => {
      ...
      await expect.element(screen.getByTestId('has-admin')).toHaveTextContent('true')
    })
  })
})
...
describe('Can component', () => {
  describe('with single permission', () => {
    ...
    it('does not render children when permission is denied', async () => {
      ...
      expect(screen.container.querySelector('[data-testid="content"]')).toBeNull()
    })
  })
  ...
  describe('with fallback', () => {
    ...
    it('does not render fallback when permission is granted', async () => {
      ...
      await expect
        .element(screen.getByTestId('content'))
        .toHaveTextContent('Authorized Content')
      expect(screen.container.querySelector('[data-testid="fallback"]')).toBeNull()
    })
...
describe('Provider re-render behavior', () => {
  it('updates context when roles change', async () => {
    ...
    await expect.element(screen.getByTestId('can-read')).toHaveTextContent('true')
    await expect.element(screen.getByTestId('can-delete')).toHaveTextContent('true')
  })
})
